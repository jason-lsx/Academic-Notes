...
> 以分贝来描述称为`波束模式校正(Beam Pattern Correction，BPC)`[^1]。
> $$
> BPC = 10 \lg (D(\theta, \phi)) = 20 \lg (R(\theta, \phi))
> $$

> `主瓣宽度、全开角`：主极大两侧第一个极小值之间的夹角。

> `束宽、波束宽度(beamwidth)、半功率角 ` $\Theta_{-3dB}$：由主极大的幅值下降 0.707 倍处两边的夹角或半功率辐射点之间的夹角。

![](../resources/Chapter2-水声学/束宽和全开角图.jpg)

> `阵列指向性增益(Array Directivity Gain)、发射方向性因子`：在发射阵远场最大响应方向上的声强与同一距离处个方向声强的平均值之比。
> $$
...
$$
由上图可以知道，$r_i \approx r - id\sin\theta$，距离差导致声波相位差。但对于分子的 $r_i$ 来说，因为 $r>>L$，所以 $r_i \approx r$。所以式（2）可以化简为：
$$
...
>    当 $N \to \infty$  时，
>    $$
>    20\lg\left\{ 20 \lg \left[ N \sin \left(\frac{3\pi}{2N}\right)\right] \right\} \approx -13.5 \ \ \mathrm{dB}
>    $$
...
如下图所示，预设的主瓣方向为 $\theta_0$，阵元为 $1, 2, \cdots, i, \cdots, N - 1$，也就是当阵元分别位于 $1', 2', \cdots, i', \cdots, N - 1'$ 时，即可产生主瓣位于 $\theta_0$ 的波束相应函数。
...
归一化之后可表示为：
$$
\left| R(\theta)  \right| = \frac{\sin \left( \frac{N \pi d (\sin \theta - \sin \theta_0)  }{\lambda} \right) }{N \sin \left( \frac{\pi d (\sin \theta - \sin \theta_0)}{\lambda} \right)}
$$
...
$$
下图显示了当波束角变化时波束响应函数的变化情况。

![](../resources/Chapter2-水声学/指向角变化时波束响应函数变化图.gif)

从上图中可以看出，当指向角变大时，主瓣的宽度逐渐变大。其实这很好理解，所有的波束都进行了平移，主瓣的位置占据了原来旁瓣的位置，所以旁瓣宽度增大了。但是在 0° 方向的旁瓣大小却是同步激发时主瓣大小的一半。
...
> 半径为 $a$  的圆形平面活塞镶嵌在无限大挡板上，活塞位于 $xoy$ 平面内，向上半空间辐射声波，声场对称于 $z$ 轴，活塞表面个点的振幅和相位相同。
...
不同 $ka$ 参数下波束的指向性图如下图所示。

![](../resources/Chapter2-水声学/不同ka下的指向图.JPG)

当 $ka$  值超过一阶贝塞尔函数的第一个根值 3.83 以后，辐射开始具有更为复杂的指向性。当 $\theta = \arcsin \frac{3.83}{ka} = \arcsin\left( 061 \frac{\lambda}{a} \right)$ 时， $D = 0$，辐射为 0，超过这个角度，辐射逐渐增加，并在某个角度达到次极大，此后辐射又逐渐减小，从而在指向图上就表现为除主瓣以外还会出现一些旁瓣，如上图中 $ka = 5$ 所示。
...
\right|
$$
其中，$\frac{\sin \frac{Nx}{2}}{N \cdot \frac{x}{2}}$ 为辛格函数 $sinc(\frac{Nx}{2})$。

对于 辛格函数，当取 $sinc(\frac{Nx}{2}) = 0.707 (\frac{Nx}{2} = 1.39)$ 时，得到天线波瓣的半功率点位，可以算出天线的 $-3 \ \mathrm{dB}$ 波束宽度 $\theta_{-3 dB}$ 为
$$
...
跟 $z$ 有关的项为 $\sin \frac{k(R - z)}{2}$，其大致图像为如图下图所示。

![](../resources/Chapter2-水声学/声压与距离关系图.jpg)

由公式和图可以看出，过渡距离为：$z_g = a^2 \lambda$。
