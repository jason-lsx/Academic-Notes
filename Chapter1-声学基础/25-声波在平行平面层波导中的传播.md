...
**`波导`：至少在一维方向上式无限制的有限介质空间。**

声波在平行平面层波导中的传播模型如图所示。声波在 $x \in [0, h],\ \ z \in [0, +\infty),\ \ y \in (- \infty, + \infty)$ 区域传播。

![](../resources/Chapter1-声学基础/平行平面层波导模型.jpg)

## 25.2 平行平面层波导声场的解

根据上图，可得平行平面层波导的波动方程为
$$
...
$$
\begin{cases}
k > \frac{n \pi}{h}， &  \text{该本征函数为正常传播的第n阶简正波} \\
...
根据式（21）的声压函数和等相位面的定义 $\phi_{pn} = \omega t - k_{zn}z = constant$，则第 $n$ 阶简正波声压相速度可表示为：
$$
c_{pn} = \left. \frac{dz}{dt} \right|_{\omega t - k_{zn}z = constant} = \frac{\omega}{k_{zn}} \\ = \frac{\omega}{\sqrt{k^2 - \left( \frac{n \pi}{h} \right)^2}} =  \frac{c_0}{\sqrt{1 - \left( \frac{n \pi}{hk} \right)^2}} \ge c_0
$$
不同阶数简正波的相速度如下图所示。
...
群速度为波包的传播速度，可表示为：
$$
c_{gn}= \left. \frac{dx}{dt} \right|_{\frac{(\omega_2 - \omega_1)t - (k_2 - k_1)x}{2} = constant} = \frac{\omega_2 - \omega_1}{k_2 - k_1} = \frac{\Delta \omega}{\Delta k} = \frac{d \omega}{dk}
$$
...
$$
根据上式可以看出，第一项为两信号频率的和，第二项为两信号频率的差（[拍频](https://baike.baidu.com/item/%E6%8B%8D%E9%A2%91/4034522?fr=aladdin)），其波形如下图所示。其拍频信号即为叠加场声信号的外包络，即 [波包](https://baike.baidu.com/item/%E6%B3%A2%E5%8C%85/5636964?fr=aladdin)。
